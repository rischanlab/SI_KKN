...
class Admin_model extends CI_Model
{
	function __construct()
	{
		...
		$query = $this->db->query("SELECT ID_PERIODE,PERIODE FROM KKN_PERIODE");

			$dropdown = array('' => 'Pilih Periode');
				foreach($query->result_array() as $r) {
				$dropdown[$r['ID_PERIODE']] = $r['PERIODE'];
				}
			return $dropdown;
		}
		...
		function to_csv($ta,$periode,$angkatan){
			...
			$query=$this->db->query("
			select A.NIM, A.NAMA, A.HP_MHS, DECODE(A.J_KELAMIN,'L','Laki-Laki','P','Perempuan') Jenis_Kelamin ,A.GOL_DARAH, DECODE(A.STATUS_KAWIN,'B','Belum Kawin','K','Kawin') Status_Perkawinan,A.ALAMAT_MHS , (B.FAK) Fakultas, B.ALAMAT_JOGJA, B.RT_JOGJA, B.DESA_JOGJA, (G.ANGKATAN) Angkatan_KKN ,(H.NM_DOSEN) Nama_DPL,(I.TANGGAL_MULAI) Mulai_KKN, (I.TANGGAL_SELESAI) Selesai_KKN, (K.PERIODE) Periode_KKN, (L.TA) TA_KKN, (C.RW) Lokasi_RW_KKN ,(C.DESA) Lokasi_Desa_KKN, (D.NM_KEC) Lokasi_Kec_KKN, (E.NM_KAB) Lokasi_Kab_KKN, (F.NM_PROP)Lokasi_prop_KKN FROM D_MAHASISWA A,KKN_MHS B,KKN_KELOMPOK C,MD_KEC D,MD_KAB E,MD_PROP F,KKN_ANGKATAN G,D_DOSEN H,KKN_PERIODE I, KKN_DETAIL_KELOMPOK J, KKN_PERIODE K, KKN_TA L WHERE A.NIM=B.NIM AND C.KD_KEC=D.KD_KEC AND C.KD_KAB=E.KD_KAB AND C.KD_PROP=F.KD_PROP AND C.ID_ANGKATAN=G.ID_ANGKATAN AND C.KD_DOSEN=H.KD_DOSEN AND G.ID_PERIODE=I.ID_PERIODE AND J.ID_KELOMPOK=C.ID_KELOMPOK AND B.NO=J.NO AND G.ID_PERIODE=K.ID_PERIODE AND G.ID_TA=L.ID_TA AND L.ID_TA =$ta AND K.ID_PERIODE=$periode AND G.ID_ANGKATAN=$angkatan");
			return $query;
